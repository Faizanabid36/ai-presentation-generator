...
export default function Presentation() {
  ...
  useEffect(() => {
    async function getPresentation() {
      let url = `${import.meta.env.VITE_API_URL}/presentation/${id}`;
    ...
    <>
      <section className='p-4 flex justify-center h-screen items-center flex-col text-4xl gap-6 '>
        {JSON.stringify(presentation) == '{}' ? (
          <p>This presentation does not exist</p>
        ) : presentation.generated == true ? (
          <PresentationViewer presentation={presentation} />
        ) : presentation.generated == false ? (
          <PresentationLoading presentation={presentation} />
        ) : (
          <p>1</p>
        )}
      </section>
